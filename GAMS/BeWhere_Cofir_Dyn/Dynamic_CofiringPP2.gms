...
/;
* unit InvestmentCoal_k$/GWh (GWh of electricity generation)

parameter CostIOMCoal(Y,P) /
$include txt_file_beaver/parameter-cost-IOM-coal.txt
/;
*unit CostIOMCoal_k$/GWh (GWh of electricity generation)
...
/;
* unit coalPrice_k$/GWh (GWh is converted from J of coal_energy)
...
/;
* unit BiomPrice_k$/GWh (GWh is converted from J of biomass heat)
...
/;
* unit SpecificCostCofir_k$/MWelec
...
CofiringCost(Y,P,TECH)$(YP(Y,P))..
 CostCofiring (Y,P,Tech) =E=  ((Generation(Y,P)/EffCof(P,Tech)*UP(Y,P,Tech))-
...
ElBioPerRM(Y,RM)..
         ElBiomassPerRM(Y,RM) =E= SUM((S,P,Tech) $(RMTe(RM,Tech) and transDistSupplyPlant(S,P) and YP(Y,P) and SRM(Y,S,RM)), EffCof(P,Tech)*BSP(Y,S,RM,P,Tech));
