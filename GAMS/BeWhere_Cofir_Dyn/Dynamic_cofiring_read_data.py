...
df_distance_sypply_plants = pd.read_excel(file_location, header=None, skiprows=2, sheet_name='Distance_grid_to_plant', usecols='C:E')
...
create_parameter_file(file_destination+'set-year-plant-relation.txt',['Y','P'],pd.DataFrame(np.vstack([[df_year.iloc[j, 0], df_plants.iloc[i, 0], df_power_generation.values[i,j]]
...
print('parameter-distance-supply-plant...')    
create_parameter_file(file_destination+'parameter-distance-supply-plant.txt',['Y','S','P','T'],pd.DataFrame(np.vstack([[df_year.iloc[yy,0], df_distance_sypply_plants.iloc[i,0], df_distance_sypply_plants.iloc[i,1], df_transport_mode.iloc[j,0], df_distance_sypply_plants.values[i,2]]
                                             for yy in range(df_year.shape[0]) 
